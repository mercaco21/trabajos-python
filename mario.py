# Ejercico  : Crear una tupla con  elementos y mostrar el tercer elemento 
# sistema_alumnos.py
...
class Alumno:
    ...
    def asignar_materia(self, materia):
        if materia.carrera != self.carrera:
            ...
            self.materias.sort(key=lambda m: m.codigo)  # mantener ordenado
    ...
    def cursa_materia(self, codigo):
        return self._busqueda_binaria_materia(codigo) is not None

    def eliminar_materia(self, codigo):
        idx = self._busqueda_binaria_materia(codigo, return_index=True)
        if idx is not None:
    ...
    def _busqueda_binaria_materia(self, codigo, return_index=False):
        izq, der = 0, len(self.materias) - 1
        while izq <= der:
            medio = (izq + der) // 2
            if self.materias[medio].codigo == codigo:
                return medio if return_index else self.materias[medio]
            elif self.materias[medio].codigo < codigo:
                izq = medio + 1
            else:
                der = medio - 1
        return None
    ...
    def mostrar_profesores(self):
        ...
        while actual:
            print(f"- {actual.nombre} {actual.apellido} (Materia: {actual.materia})")
            actual = actual.sig
...
if __name__ == "__main__":
    a1 = Alumno("Sofía", "Martínez", "12345678", "A100", "Ingeniería", "sofia@unab.edu.ar", "2022-03-10")

    m1 = Materia("MAT101", "Álgebra", "A", 1, "Ingeniería")
    m2 = Materia("MAT102", "Análisis", "A", 1, "Ingeniería")
    m3 = Materia("FIS101", "Física", "B", 2, "Ingeniería")
    m4 = Materia("DIS101", "Diseño I", "C", 1, "Diseño")

    a1.asignar_materia(m1)
    a1.asignar_materia(m2)
    a1.asignar_materia(m3)
    a1.asignar_materia(m4)  # debe ser rechazada

    a1.mostrar_materias()

    print("\n¿Cursa MAT102?", a1.cursa_materia("MAT102"))
    print("¿Cursa BIO101?", a1.cursa_materia("BIO101"))

    a1.eliminar_materia("MAT101")
    a1.mostrar_materias()

    a1.materias_aprobadas = ["MAT102", "FIS101"]
    try:
        print(a1.verificar_aprobada("MAT102"))
        print(a1.verificar_aprobada("MAT999"))
    except Exception as e:
        print("Excepción:", e)

    # Profesores
    p1 = Profesor("Laura", "García", "Álgebra")
    p2 = Profesor("Carlos", "Méndez", "Física")
    p3 = Profesor("Ana", "Ruiz", "Análisis")

    a1.agregar_profesor(p1)
    a1.agregar_profesor(p2)
    a1.agregar_profesor(p3)

    a1.mostrar_profesores()
